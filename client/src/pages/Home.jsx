  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        ...
        setFashionPosts(posts.filter(post => post.category === "fashion"));
        setMakeupPosts(posts.filter(post => post.category === "makeup"));
        setLifestylePosts(posts.filter(post => post.category === "lifestyle"));
      } catch (error) {
        console.log(error);
      }
    };
  ...
  return (
    <div className="bg-white text-gray-900">
          ...
          {events.slice(0, 3).map(post => (
            ...
            >
              ...
              <div className="p-6">
                ...
                <p className="text-gray-500 mt-3 leading-relaxed">{post.subtitle}</p>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section className="max-w-7xl mx-auto px-5 sm:px-8 py-20">
        <div className="flex flex-col sm:flex-row justify-between sm:items-end mb-12 gap-5">
          <div>
            <h2 className="text-4xl md:text-5xl font-serif text-[#011145]">Blog</h2>
            <p className="text-gray-500 mt-3 text-base md:text-lg">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          {fashionPosts.slice(0, 1).map((post) => (
            ...
            >
              ...
              <div className="p-6">
                <span className="text-xs uppercase tracking-widest font-semibold text-gray-400 block mb-2">Fashion</span>
                <h3 className="text-2xl font-serif text-[#011145]">{post.title}</h3>
                <p className="text-gray-500 mt-3 leading-relaxed line-clamp-2">{post.subtitle || post.excerpt}</p>
              </div>
            </Link>
          ))}
          ...
          {makeupPosts.slice(0, 1).map((post) => (
            ...
            >
              ...
              <div className="p-6">
                <span className="text-xs uppercase tracking-widest font-semibold text-gray-400 block mb-2">Beauty</span>
                <h3 className="text-2xl font-serif text-[#011145]">{post.title}</h3>
                <p className="text-gray-500 mt-3 leading-relaxed line-clamp-2">{post.subtitle || post.excerpt}</p>
              </div>
            </Link>
          ...
          {lifestylePosts.slice(0, 1).map((post) => (
            ...
            >
              ...
              <div className="p-6">
                <span className="text-xs uppercase tracking-widest font-semibold text-gray-400 block mb-2">Lifestyle</span>
                <h3 className="text-2xl font-serif text-[#011145]">{post.title}</h3>
                <p className="text-gray-500 mt-3 leading-relaxed line-clamp-2">{post.subtitle || post.excerpt}</p>
              </div>
            </Link>
      ...
      <section className="w-full pt-20 pb-0 text-center">
        <div className="max-w-3xl mx-auto px-5 sm:px-8 mb-14">
          <span className="text-xs uppercase tracking-[0.3em] font-medium text-gray-400 block mb-3">Creative Portfolio</span>
          <h2 className="text-4xl md:text-5xl font-serif text-[#000b21] tracking-wide mb-4">Photoshoot Editorials</h2>
        ...
        <div className="w-full bg-[#00071c] text-white py-16 md:py-24 px-6 border-y border-white/10 shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-2xl mx-auto flex flex-col items-center">
            <span className="text-[11px] uppercase tracking-[0.35em] text-gray-400 font-medium mb-4 border border-white/15 px-4 py-1.5">
              Next Season
            </span>
            <h3 className="text-3xl md:text-5xl font-serif font-normal tracking-wide text-white mb-6">Coming Soon</h3>
      ...
      <section className="bg-[#eef2f6] text-[#011145] py-20 px-6 sm:px-12">
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-12 items-start">
          ...
          {/* Left Column: Graphic Card + Membership Buttons */}
          <div className="md:col-span-5 flex flex-col items-center">
            ...
            <a
              ...
              className="mt-8 border-2 border-[#011145] text-[#011145] px-8 py-3 text-sm font-medium hover:bg-[#011145] hover:text-white transition duration-300 text-center cursor-pointer w-full"
            >
              Membership Here!
            </a>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="mt-4 border-2 border-[#011145] text-[#011145] px-8 py-3 text-sm font-medium hover:bg-[#011145] hover:text-white transition duration-300 text-center cursor-pointer w-full"
            >
              Join Our Committee
            </a>
          </div>
...
for (const pair of formData.entries()) {
  console.log(pair[0], pair[1]);
            ...
            >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <input
                      ...
                      name="first_name"
                      required
                      className="w-full border border-[#011145]/30 bg-white/50 focus:bg-white p-3 text-sm focus:outline-none focus:border-[#011145] transition"
                    />
                    <span className="text-xs text-gray-500 mt-1 block">First Name <span className="text-gray-400">(required)</span></span>
                  </div>
                  <div>
                    <input
                      type="text"
                      ...
                      className="w-full border border-[#011145]/30 bg-white/50 focus:bg-white p-3 text-sm focus:outline-none focus:border-[#011145] transition"
                    />
                    <span className="text-xs text-gray-500 mt-1 block">Last Name <span className="text-gray-400">(required)</span></span>
                  </div>
                </div>
              </div>

              <div>
                <label className="block text-sm font-medium mb-1">Email <span className="text-xs text-gray-500 font-normal">(required)</span></label>
                <input
                  type="email"
                  name="email"
                  required
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Subject <span className="text-xs text-gray-500 font-normal">(required)</span></label>
                <input
                  type="text"
                  name="subject"
                  required
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Message <span className="text-xs text-gray-500 font-normal">(required)</span></label>
                <textarea
                  name="message"
                  rows={5}
                  required
              ...
              <div>
                <button
                  type="submit"
                  className="border border-[#011145] text-[#011145] px-8 py-2.5 text-sm font-medium hover:bg-[#011145] hover:text-white transition duration-300 disabled:opacity-50"
                >
                  Submit
                </button>
              </div>
